...
async def register_player(account_id: str, player_id: str, player_data: PlayerCreate):
    ...
    player = store.accounts.register_player(account_id, player_id, player_dict)
...
async def get_player(account_id: str, player_id: str):
    ...
    player = store.accounts.get_player(account_id, player_id)
    if player is None:
...
async def list_players(account_id: str, page: int = 1, per_page: int = 10):
    ...
    return store.accounts.list_players(account_id, page, per_page)
