...
def test_account_partial_update_preserves_name_when_same_value(client):
    """Update account with same name (idempotent); ensure value unchanged and no duplicates."""
    put_json(client, "/v1/accounts/testuser1", {"name": "Test User 1"})
    data = get_json(client, "/v1/accounts")
    assert any(item["id"] == "testuser1" and item["name"] == "Test User 1" for item in data["items"])
